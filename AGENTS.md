...
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release
```

**Run a single test method**:
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release -- --filter-method ClassName.MethodName
```

**Run all tests in a test class**:
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release -- --filter-class ClassName
```

**Run tests with wildcard matching** (supports wildcards at beginning and/or end):
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release -- --filter-method "*Pattern*"
```

**Run tests with a specific trait** (equivalent to category filtering):
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release -- --filter-trait "TraitName=value"
```

**Exclude tests with a specific trait** (skip unstable tests):
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release -- --filter-not-trait "TestCategory=FailsInCloudTest"
```

**Run tests for a specific framework only**:
```bash
dotnet test --project test/Nerdbank.Algorithms.Tests/Nerdbank.Algorithms.Tests.csproj --no-build -c Release --framework net9.0
```

**List all available tests without running them**:
```bash
cd test/Nerdbank.Algorithms.Tests
