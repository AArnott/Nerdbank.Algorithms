...
namespace NerdBank.Tools.Exslt
{
	...
	public class ExsltMath {
		...
		/// node set</returns>
		/// <remarks>THIS FUNCTION IS NOT PART OF EXSLT!!!</remarks>
		public double avgNonNegatives(XPathNodeIterator iterator)
		{
			...
			{ 
				while(iterator.MoveNext())
				{
					double val = XmlConvert.ToDouble(iterator.Current.Value);
					if( val >= 0 )
						sum += val;
					else // ignore negatives
						count--; // don't count against it in averaging
				}
