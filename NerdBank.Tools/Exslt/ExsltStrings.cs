...
namespace NerdBank.Tools.Exslt {
	...
	public class ExsltStrings {
		...
		public XPathNodeIterator tokenize(string str, string delimiters){					
			...
			doc.AppendChild(doc.CreateElement("tokens"));
			 
			string[] tokens = Tokenize(str, delimiters.ToCharArray());
			...
			// Use an XPathDocument to create the navigator, to avoid Microsoft
			// bug that is way too difficult to explain, but is exploited by stacked grids 
			// with 3 variables per row or more.
			XPathDocument xpathDoc = new XPathDocument( new System.IO.StringReader(doc.OuterXml) );
			return xpathDoc.CreateNavigator().Select("/tokens/token"); 
		}

		protected static string[] Tokenize(string str, char[] delimiters)
		{
			string[] tokens = str.Split(delimiters); // tokenize around whitespace
