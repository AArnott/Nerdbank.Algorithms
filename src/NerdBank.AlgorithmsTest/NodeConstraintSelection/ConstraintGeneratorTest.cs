...
public class ConstraintGeneratorTest
{
	...
	public void StrictSubsetConstraintIncludesEntireRequirementTest()
	{
		...
		var nodes = new INode[] { new DummyNode(), new DummyNode(), new DummyNode(), new DummyNode(), new DummyNode() };
		...
		Assert.Single(deduced);
		var c = deduced[0] as SelectionCountConstraint;
		Assert.NotNull(c);
		Assert.Equal(0, c.Max);
		Assert.True(c.SelectionState);
		var expectedNodes = new INode[] { nodes[0], nodes[4] };
		INode[] actualNodes = c.Nodes.ToArray();
		Assert.Equal<INode>(expectedNodes, actualNodes);
	}
	...
	public void MinimumConstraintsFillExactConstraintTest()
	{
		...
		Assert.Single(deduced);
		SelectionCountConstraint e = Assert.IsType<SelectionCountConstraint>(deduced[0]);
		Assert.Equal(0, e.Max);
		Assert.True(e.SelectionState);
		var expectedNodes = new INode[] { nodes[2] };
	...
	public void CombinationEliminationTest()
	{
		...
		Assert.Equal(0, e.Max);
		Assert.True(e.SelectionState);
		var expectedNodes = new INode[] { nodes[4] };
		INode[] actualNodes = e.Nodes.ToArray();
		Assert.Equal<INode>(expectedNodes, actualNodes);
	}
}
